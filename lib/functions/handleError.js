...
function logError(error, extraInfo) {
    ...
    request({
        url: 'https://gas-manage.firebaseio.com/logs/errors.json',
        method: "POST",
        json: true,
        headers: {
        ...
        getProjectRoot('.').then((value) => {
            if (value.found) {
                ...
                if (syntaxErrors.length > 0) {
                    ...
                    console.log(`Google doesn't accept the code you are trying to push. Try doublechecking for syntax errors. Push was reverted. [${`✘`.red}]`);
                    extraInfo = `Syntax error + unsure + root`;
                }
            } else {
                console.log(`Google doesn't accept the code you are trying to push. Try doublechecking for syntax errors. Push was reverted. [${`✘`.red}]`);
